...
// Function to calculate memory usage in bytes for wordList
size_t calcMemoryUsage(WordFreq wordList[]) {
    size_t totalMemory = 0;

    // Calculate memory for each word in wordList (including its frequency and string length)
    for (int i = 0; i < wordCount; i++) {
        totalMemory += sizeof(wordList[i].frequency);  // Memory for frequency (int)
        totalMemory += wordList[i].word.capacity();    // Memory for the string (word)
    }
